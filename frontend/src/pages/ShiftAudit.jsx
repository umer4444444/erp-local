  ...
  return (
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-main)', fontFamily: "'Outfit', sans-serif" }}>
      <header style={{ marginBottom: 40, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 32, fontWeight: 900, color: 'var(--text-main)' }}>Shift Audit</h1>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <button
            ...
            style={{ padding: '12px 24px', borderRadius: 16, background: auditData.length === 0 ? 'var(--border-color)' : '#0a84ff', color: auditData.length === 0 ? 'var(--text-muted)' : 'var(--bg-panel)', border: 'none', fontWeight: 800, cursor: auditData.length === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 40 }}>
        {[
          ...
          <div key={i} style={{ background: 'var(--bg-panel)', padding: 24, borderRadius: 24, border: '1px solid var(--border-color-rgb)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)' }}>{stat.label}</div>
            </div>
            <div style={{ fontSize: 28, fontWeight: 900, color: 'var(--text-main)' }}>{stat.value}</div>
          </div>
        ))}
      </div>
            ...
            <thead>
              <tr style={{ textAlign: 'left', borderBottom: '2px solid #f1f5f9' }}>
                {['EMPLOYEE', 'DATE', 'CLOCK IN', 'CLOCK OUT', 'HOURS', 'OVERTIME', 'RATE', 'EARNED'].map(h => (
                  <th key={h} style={{ padding: '16px', color: 'var(--text-muted)', fontSize: 12, fontWeight: 800 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan="8" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>Loading audit data...</td></tr>
              ) : auditData.length === 0 ? (
                <tr><td colSpan="8" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>No shift records found for this period.</td></tr>
              ) : (
                  ...
                  return (
                    <tr key={i} onClick={() => setSelectedRow(item)}
                      ...
                      <td style={{ padding: '16px' }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700 }}>{item.empCode}</div>
                      </td>
                      ...
                      <td style={{ padding: '16px', fontWeight: 800, color: 'var(--text-main)' }}>{item.hours}h</td>
                      <td style={{ padding: '16px' }}>
                        {overtime > 0
                      ...
                      <td style={{ padding: '16px', fontSize: 13, fontWeight: 700, color: 'var(--text-muted)' }}>
        ...
        {selectedRow && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }}
              style={{ background: 'var(--bg-panel)', borderRadius: 28, width: '100%', maxWidth: 480, position: 'relative', padding: 36, zIndex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
                <h2 style={{ fontSize: 20, fontWeight: 900, color: 'var(--text-main)' }}>Shift Details</h2>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  ...
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', borderRadius: 12, background: 'var(--bg-main)' }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)' }}>{label}</span>
                    <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text-main)' }}>{value}</span>
                  </div>
                ))}
              </div>
            </motion.div>
