        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: 1 }}>Field Sales Logistics</span>
            </div>
            ...
            {filtered.map(delivery => {
              ...
              return (
                <tr key={delivery.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                  <td style={{ padding: '14px 20px', fontWeight: 800, color: '#0f172a', fontSize: 14 }}>{customer.shopName || 'Walk-in Customer'}</td>
                  <td style={{ padding: '14px 20px', color: '#64748b', fontWeight: 600, fontSize: 13 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {delivery.status !== 'DELIVERED' ? (
                        <button onClick={() => { setSelectedDelivery(delivery); setShowProofModal(true); }}
                          style={{ padding: '6px 12px', borderRadius: 8, background: '#10b981', color: 'white', border: 'none', fontWeight: 800, fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 40 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }}
              ...
              <div style={{ background: '#f8fafc', borderRadius: 16, padding: 16, marginBottom: 24 }}>
                <div style={{ fontWeight: 800, color: '#0f172a', marginBottom: 4 }}>{selectedDelivery.Sale?.Customer?.shopName || 'Customer'}</div>
              ...
              <div style={{ marginBottom: 24 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: '#64748b', marginBottom: 10 }}>CUSTOMER SIGNATURE</label>
              ...
              <button onClick={submitProof} disabled={submittingProof}
                style={{ width: '100%', padding: 18, borderRadius: 16, background: '#10b981', color: 'white', border: 'none', fontWeight: 900, cursor: 'pointer', fontSize: 16 }}>
