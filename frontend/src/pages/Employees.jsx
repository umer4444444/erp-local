...
const cardStyle = {
  background: 'var(--bg-panel)', borderRadius: 20, padding: 24,
  border: '1px solid rgba(0,0,0,0.06)',
  boxShadow: '0 4px 16px var(--border-color-rgb)',
};

const inputStyle = {
  width: '100%', padding: '12px 14px', border: '1.5px solid #e2e8f0',
  borderRadius: 12, outline: 'none', fontFamily: "'Outfit', sans-serif",
  fontSize: 14, fontWeight: 600, color: 'var(--text-main)', background: 'var(--bg-main)',
};
...
const Employees = () => {
  ...
  const getStatusColor = (status) => {
    switch(status) {
      ...
      case 'inactive': return { bg: '#f1f5f9', color: 'var(--text-muted)' };
  ...
  return (
    <div style={{ padding: '36px 40px', minHeight: '100vh', background: 'var(--bg-main)' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 900, color: 'var(--text-main)' }}>Employee Management</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: 13, fontWeight: 600 }}>Manage your global workforce</p>
        </div>
        <button onClick={() => setShowModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 22px', borderRadius: 12, background: '#0a84ff', color: 'var(--bg-panel)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>
        ...
        {departments.filter(d => d.employeeCount > 0).map(dept => (
          <div key={dept.id} style={{ ...cardStyle, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{dept.employeeCount} Employees</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ ...cardStyle, overflow: 'hidden', padding: 0 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          {selectedIds.size > 0 && (
            <button style={{ marginLeft: 'auto', padding: '12px 16px', borderRadius: 12, background: 'var(--border-color)', border: 'none', fontWeight: 800, cursor: 'pointer' }}>
        ...
        <div className="w-full overflow-x-auto"><table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: 'var(--bg-main)', borderBottom: '1px solid #f1f5f9' }}>
              ...
              <th style={{ padding: 16, textAlign: 'left', fontSize: 11, fontWeight: 900, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Employee</th>
              <th style={{ padding: 16, textAlign: 'left', fontSize: 11, fontWeight: 900, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Department</th>
              <th style={{ padding: 16, textAlign: 'left', fontSize: 11, fontWeight: 900, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Status</th>
              <th style={{ padding: 16, textAlign: 'right', fontSize: 11, fontWeight: 900, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Actions</th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={emp.id} style={{ borderBottom: '1px solid #f8fafc', background: selectedIds.has(emp.id) ? '#eff6ff' : 'transparent' }}>
                ...
                <td style={{ padding: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800 }}>{(emp.firstName || '')[0]}{(emp.lastName || '')[0]}</div>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 14 }}>{emp.firstName} {emp.lastName}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{emp.empCode}</div>
                    </div>
                  </div>
                </td>
      ...
      <AnimatePresence>
        {showModal && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }} style={{ background: 'var(--bg-panel)', borderRadius: 24, width: '100%', maxWidth: 500, position: 'relative', padding: 32 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h2 style={{ fontSize: 20, fontWeight: 900 }}>Add New Employee</h2>
                <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
                  {[1, 2, 3, 4, 5].map(s => (
                    <div key={s} style={{ height: 6, width: 24, borderRadius: 3, background: step >= s ? '#0a84ff' : 'var(--border-color)', transition: 'all 0.3s' }} />
                  ))}
              ...
              <form onSubmit={handleSave}>
                {step === 1 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <button type="button" onClick={() => setStep(2)} style={{ padding: 14, borderRadius: 12, background: '#0a84ff', color: 'var(--bg-panel)', fontWeight: 800, border: 'none', marginTop: 12 }}>Continue</button>
                  </div>
                )}
                {step === 2 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 4 }}>Select Department (determines portal access)</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, maxHeight: 320, overflowY: 'auto' }}>
                      {departments.map(d => {
                        ...
                        const getDeptInfo = (name) => {
                          ...
                          if (n.includes('eod') || n.includes('operation')) return { portal: 'Operations Hub', role: 'operations', color: 'var(--text-muted)', isAdmin: false };
                          ...
                          return { portal: 'Staff Portal', role: 'staff', color: 'var(--text-muted)', isAdmin: false };
                        };
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              background: isSelected ? `${info.color}10` : 'var(--bg-panel)',
                            }}
                          >
                            <div style={{ fontWeight: 800, fontSize: 13, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: 6 }}>
                              {d.name}
                              {info.isAdmin && <span style={{ fontSize: 9, fontWeight: 900, background: '#6366f1', color: 'var(--bg-panel)', padding: '2px 5px', borderRadius: 4 }}>ADMIN</span>}
                            </div>
                            <div style={{ fontSize: 11, fontWeight: 600, color: info.color, marginTop: 2 }}>→ {info.portal}</div>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', marginTop: 2 }}>Role: {info.role}</div>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                      <button type="button" onClick={() => setStep(1)} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#f1f5f9', color: 'var(--text-muted)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>← Back</button>
                      <button type="button" onClick={() => setStep(3)} disabled={!form.departmentId} style={{ flex: 2, padding: 14, borderRadius: 12, background: form.departmentId ? '#0a84ff' : 'var(--border-color)', color: form.departmentId ? 'var(--bg-panel)' : 'var(--text-muted)', fontWeight: 800, border: 'none', cursor: form.departmentId ? 'pointer' : 'not-allowed' }}>Continue →</button>
                    </div>
                  </div>
                )}
                {step === 3 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                      <button type="button" onClick={() => setStep(1)} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#f1f5f9', color: 'var(--text-muted)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>Edit Basic Info</button>
                      <button type="button" onClick={() => setStep(2)} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#f1f5f9', color: 'var(--text-muted)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>← Back</button>
                      <button type="button" onClick={() => setStep(4)} style={{ flex: 2, padding: 14, borderRadius: 12, background: '#0a84ff', color: 'var(--bg-panel)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>Continue →</button>
                    </div>
                  </div>
                )}
                {step === 4 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 4 }}>Documents & Expiries</div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Iqama Expiry</label>
                        <input type="date" style={inputStyle} value={form.iqamaExpiryDate} onChange={e => setForm({...form, iqamaExpiryDate: e.target.value})} />
                      </div>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Contract Expiry</label>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Vehicle Rent Expiry</label>
                      ...
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>SIM Expiry</label>
                        <input type="date" style={inputStyle} value={form.simExpiryDate} onChange={e => setForm({...form, simExpiryDate: e.target.value})} />
                      </div>
                    </div>

                    <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                      <button type="button" onClick={() => setStep(3)} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#f1f5f9', color: 'var(--text-muted)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>← Back</button>
                      <button type="button" onClick={() => setStep(5)} style={{ flex: 2, padding: 14, borderRadius: 12, background: '#0a84ff', color: 'var(--bg-panel)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>Continue to Face Scan →</button>
                    </div>
                  </div>
                )}
                {step === 5 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 4 }}>Face Scanning (Attendance)</div>
                    <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      We need to register your face for the attendance system. Please ensure your face is clearly visible.
                    </p>
                    
                    <div style={{ background: 'var(--bg-main)', padding: 20, borderRadius: 12, border: '2px dashed #e2e8f0', textAlign: 'center' }}>
                       {form.faceDescriptor ? (
                         <div style={{ color: '#10b981', fontWeight: 800 }}>✓ Face Scanned Successfully</div>
                       ) : (
                         <button type="button" onClick={() => setShowFaceScanner(true)} style={{ padding: '12px 24px', borderRadius: 8, background: '#3b82f6', color: 'var(--bg-panel)', border: 'none', cursor: 'pointer', fontWeight: 700 }}>
                           Open Camera to Scan
                         </button>
                       )}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                      <button type="button" onClick={() => setStep(4)} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#f1f5f9', color: 'var(--text-muted)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>← Back</button>
                      <button type="submit" style={{ flex: 2, padding: 14, borderRadius: 12, background: '#10b981', color: 'var(--bg-panel)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>✓ Complete & Add Employee</button>
                    </div>
                  </div>
                )}
              </form>
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }} style={{ background: 'var(--bg-panel)', borderRadius: 24, width: '100%', maxWidth: 500, position: 'relative', padding: 32 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                <div style={{ width: 64, height: 64, borderRadius: 16, background: 'var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 900 }}>
                  {(selectedEmployee.firstName || '')[0]}
                </div>
                <div>
                  <h3 style={{ fontSize: 20, fontWeight: 900 }}>{selectedEmployee.firstName} {selectedEmployee.lastName}</h3>
                  <div style={{ color: 'var(--text-muted)', fontWeight: 600 }}>{selectedEmployee.empCode}</div>
                </div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Department</div>
                  <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>{selectedEmployee.Department?.name || '—'}</div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Designation</div>
                  <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>{selectedEmployee.Designation?.name || '—'}</div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Email</div>
                  <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>{selectedEmployee.email}</div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Phone</div>
                  <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>{selectedEmployee.phone || '—'}</div>
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Salary</div>
                    {!editingSalary ? (
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={handleSaveSalary} style={{ fontSize: 11, fontWeight: 800, color: 'var(--bg-panel)', background: '#10b981', border: 'none', borderRadius: 8, padding: '4px 10px', cursor: 'pointer' }}>Save</button>
                        <button onClick={() => setEditingSalary(false)} style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-muted)', background: '#f1f5f9', border: 'none', borderRadius: 8, padding: '4px 10px', cursor: 'pointer' }}>Cancel</button>
                      </div>
                    )}
                  </div>
                  {editingSalary ? (
                    ...
                    <div style={{ fontWeight: 600, color: 'var(--text-main)', marginTop: 4 }}>SAR {parseFloat(selectedEmployee.salary || 0).toLocaleString()} <span style={{ fontSize: 12, color: 'var(--text-muted)', textTransform: 'capitalize' }}>({selectedEmployee.salaryType})</span></div>
                  )}
                </div>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Status</div>
              ...
              {(user?.role === 'admin' || user?.role === 'hr' || user?.role === 'manager') && (
                <div style={{ marginTop: 24, paddingTop: 24, borderTop: '1px solid #f1f5f9' }}>
                  ...
                  {loadingLeaves ? (
                    <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Loading leaves...</div>
                  ) : (
                    <div style={{ display: 'grid', gap: 16 }}>
                      {empLeaveBalance.length > 0 && (
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          {empLeaveBalance.map(b => (
                            <div key={b.id} style={{ background: 'var(--bg-main)', padding: '8px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 12 }}>
                              <span style={{ fontWeight: 800, color: 'var(--text-main)' }}>{b.type.toUpperCase()}:</span> {b.used}/{b.total} used
                            </div>
                          ))}
                        </div>
                      )}
                      {empLeaves.length > 0 ? (
                        <div className="w-full overflow-x-auto"><table style={{ width: '100%', fontSize: 13, textAlign: 'left', borderCollapse: 'collapse' }}>
                          <thead>
                          ...
                          <tbody>
                            {empLeaves.slice(0, 5).map(l => (
                              <tr key={l.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                                ...
                                <td style={{ padding: '8px 0', color: 'var(--text-muted)' }}>{new Date(l.startDate).toLocaleDateString()} - {new Date(l.endDate).toLocaleDateString()}</td>
                        ...
                        <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>No leave requests found for this employee.</div>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }} style={{ background: 'var(--bg-panel)', borderRadius: 24, width: '100%', maxWidth: 400, position: 'relative', padding: 32 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setResetPassModal({ open: false, employee: null, newPassword: '' })} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#f1f5f9', color: 'var(--text-muted)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>Cancel</button>
                <button onClick={handleResetPassword} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#0a84ff', color: 'var(--bg-panel)', fontWeight: 800, border: 'none', cursor: 'pointer' }}>Reset Password</button>
              </div>
            </motion.div>
          </div>
        )}
