...
const MobileProfile = () => {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-main)', margin: '0 0 20px' }}>Profile</h2>
      
      <div style={{ background: 'var(--bg-panel)', borderRadius: 16, padding: 24, boxShadow: '0 2px 8px var(--border-color-rgb)', display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 24 }}>
        ...
        <h3 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: 'var(--text-main)' }}>{user?.name}</h3>
        <p style={{ margin: '4px 0 0', color: 'var(--text-muted)', textTransform: 'capitalize' }}>{user?.role?.replace('_', ' ')}</p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <button style={{ background: 'var(--bg-panel)', border: '1px solid #e2e8f0', padding: 16, borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left', fontWeight: 600, color: '#334155' }}>
          <Bell size={20} color='var(--text-muted)' /> Notifications
        </button>
        <button style={{ background: 'var(--bg-panel)', border: '1px solid #e2e8f0', padding: 16, borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left', fontWeight: 600, color: '#334155' }}>
